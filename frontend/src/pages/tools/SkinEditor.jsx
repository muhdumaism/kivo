...
import { PenTool, Download, UploadCloud, Eraser } from 'lucide-react';
...
export default function SkinEditor() {
  ...
  // Initialize empty transparent 64x64 canvas
  useEffect(() => {
    ...
    // Fill with default Steve if no skin is loaded (for visual reference)
    // Actually, let's just let it be transparent. Minecraft handles transparent skins by making them black, but we can upload a base skin.
    // Let's just create a blank texture initially.
    updatePreview();
  }, []);

  const updatePreview = () => {
    if (canvasRef.current) {
      setSkinDataUrl(canvasRef.current.toDataURL('image/png'));
    }
  };
  ...
  const drawPixel = (e) => {
    ...
    // Calculate pixel coordinates (canvas is 64x64, but displayed larger)
        ...
        <div className="mb-12">
          <div className="flex items-center gap-3 mb-4">
            <div className="w-12 h-12 bg-[#24201A] rounded-xl flex items-center justify-center">
              <PenTool className="w-6 h-6 text-[#F5C542]" />
            </div>
            <h1 className="text-4xl font-heading font-black text-[#FFF8E1] uppercase tracking-tight">Skin Editor</h1>
          </div>
          <p className="text-[#FFF8E1]/60 font-mono text-lg">
            Paint directly on the 2D texture map and see it update instantly on the 3D model.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-3 space-y-6">
            ...
            <div className="bg-[#24201A] border border-[#92400E] rounded-3xl p-6">
              ...
              >
                <UploadCloud className="w-4 h-4" /> Load Skin PNG
              </button>
              ...
              >
                <Download className="w-4 h-4" /> Export Skin
              </button>
            </div>
          </div>

          {/* 2D Canvas Editor */}
          <div className="lg:col-span-5 bg-[#171512] border border-[#92400E]/50 rounded-3xl p-6 flex flex-col items-center relative overflow-hidden group">
            <h3 className="absolute top-6 left-6 text-xs font-bold font-mono tracking-widest uppercase text-[#FFF8E1]/50 z-10">2D Texture (64x64)</h3>
            <div className="absolute inset-0 bg-[url('https://minecraft.wiki/images/Stone_Bricks.png')] opacity-5 bg-repeat"></div>
            
            <div className="relative mt-8 bg-[url('data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAIAAAACCAYAAABytg0kAAAAFElEQVQIW2NkYGD4z8DAwMgAI0AMDA4wB9/6YVQAAAAASUVORK5CYII=')] bg-repeat shadow-[0_0_0_1px_rgba(245,197,66,0.3)] touch-none select-none" style={{ backgroundSize: '16px 16px' }}>
              <canvas
                ref={canvasRef}
                width={64}
                height={64}
                className="w-[400px] h-[400px] [image-rendering:pixelated] cursor-crosshair opacity-90 hover:opacity-100 relative z-10"
            ...
            <p className="text-[#FFF8E1]/40 font-mono text-[10px] mt-6 relative z-10 text-center max-w-[300px]">
              Use standard Minecraft skin mapping layout. 64x64 pixels.
            </p>
          </div>
