...
export function ShaderBackground() {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // Pack uniforms exactly matching the spec:
      // u_scene: resolution.xy, time (seconds * 0.73), colorCount (4.0)
      gl.uniform4f(uSceneLoc, canvas.width, canvas.height, elapsedSeconds * 0.73, 4.0);

      // u_shape: scale (1.16), intensity (0.34), paramA (0.50), warp (0.00)
      gl.uniform4f(uShapeLoc, 1.16, 0.34, 0.50, 0.00);

      // u_surface: detail (2.40), contrast (1.16), brightness (0.00), saturation (1.00)
      gl.uniform4f(uSurfaceLoc, 2.40, 1.16, 0.00, 1.00);

      // u_finish: hue (0.00), vignette (0.00), blur (0.00), grain (0.09)
      gl.uniform4f(uFinishLoc, 0.00, 0.00, 0.00, 0.09);

      // u_transform: seed (1453.0), rotation (0.00), drift (0.00), oklab (1.00)
      gl.uniform4f(uTransformLoc, 1453.0, 0.00, 0.00, 1.00);

      // u_space: offset.xy (0.0, 0.0), pointer.xy (0.0, 0.0)
      gl.uniform4f(uSpaceLoc, 0.00, 0.00, 0.00, 0.00);

      // u_cursor: presence (0.00 [off]), effect (2.0), strength (0.65), radius (0.46)
      gl.uniform4f(uCursorLoc, 0.00, 2.0, 0.65, 0.46);

      // Set palette colors
      gl.uniform3fv(uColorsLoc, colorValues);
