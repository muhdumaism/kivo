...
def test_register_success_and_defaults():
    email = f"test_reg_{uuid.uuid4().hex[:8]}@qiveoqa.io"
...
def test_register_duplicate_email():
    r = requests.post(f"{API}/auth/register", json={
        "email": "creator@qiveo.dev", "password": "TestPass!2026", "name": "Dup"}, timeout=30)
...
def test_register_validation():
    ...
    assert requests.post(f"{API}/auth/register", json={
        "email": f"test_{uuid.uuid4().hex[:6]}@qiveoqa.io", "password": "123", "name": "Bad"}, timeout=30).status_code == 422
...
def test_register_age_confirm_enforced():
    ...
    email = f"test_age_{uuid.uuid4().hex[:8]}@qiveoqa.io"
    r = requests.post(f"{API}/auth/register", json={
...
def test_login_unknown_email():
    r = requests.post(f"{API}/auth/login", json={"email": "nobody@qiveoqa.io", "password": "x"}, timeout=30)
    assert r.status_code == 401
...
def test_brute_force_lockout():
    ...
    email = f"test_bf_{uuid.uuid4().hex[:8]}@qiveoqa.io"
