...
@pytest.fixture(scope="session")
def creator_session():
    return _session(_login({"email": "creator@kivo.dev", "password": "Creator!2026"}))


@pytest.fixture(scope="session")
def staff_sessions():
    out = {}
    for email in ["mod@kivo.dev", "reviewer@kivo.dev", "support@kivo.dev", "auditor@kivo.dev"]:
...
def new_user_session():
    ...
    email = f"test_newuser_{uuid.uuid4().hex[:8]}@kivoqa.io"
