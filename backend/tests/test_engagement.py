...
def test_review_requires_auth():
    assert requests.post(f"{API}/mods/aether-knight/reviews",
                         json={"rating": 5, "body": "x"}, timeout=30).status_code == 401


def test_add_review_updates_rating(admin_session):
    email = f"test_rev_{uuid.uuid4().hex[:8]}@qiveoqa.io"
...
def test_rating_aggregation_blends_seeded_baseline():
    """Posting one review must blend into the seeded baseline (base_rating_sum/count),
    not reset social proof to a single rating."""
    slug = "ember-fox"
    before = requests.get(f"{API}/mods/{slug}", timeout=30).json()
    email = f"test_agg_{uuid.uuid4().hex[:8]}@qiveoqa.io"
    reg = requests.post(f"{API}/auth/register", json={
        "email": email, "password": "TestPass!2026", "name": "TEST_Agg"}, timeout=30).json()
    s = requests.Session()
    s.headers.update({"Authorization": f"Bearer {reg['token']}"})
    r = s.post(f"{API}/mods/{slug}/reviews", json={"rating": 1, "body": "TEST low"}, timeout=30)
    assert r.status_code == 200, r.text[:200]
    detail = requests.get(f"{API}/mods/{slug}", timeout=30).json()
    assert detail["rating_count"] == before["rating_count"] + 1
    # a single 1-star review must not tank a highly rated drop
    assert detail["rating_avg"] > before["rating_avg"] - 0.2


def test_add_comment_and_list():
    email = f"test_cmt_{uuid.uuid4().hex[:8]}@qiveoqa.io"
    reg = requests.post(f"{API}/auth/register", json={
        "email": email, "password": "TestPass!2026", "name": "TEST_Commenter"}, timeout=30).json()
    s = requests.Session()
    s.headers.update({"Authorization": f"Bearer {reg['token']}"})
    slug = "nether-gate-reforged"
...
def test_empty_comment_rejected():
    email = f"test_ec_{uuid.uuid4().hex[:8]}@qiveoqa.io"
    reg = requests.post(f"{API}/auth/register", json={
        "email": email, "password": "TestPass!2026", "name": "TEST_Empty"}, timeout=30).json()
    s = requests.Session()
    s.headers.update({"Authorization": f"Bearer {reg['token']}"})
    r = s.post(f"{API}/mods/nether-gate-reforged/comments", json={"body": "   "}, timeout=30)
