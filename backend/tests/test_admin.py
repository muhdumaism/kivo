...
def test_users_list_and_trust_updates(admin_session, new_user_session):
    ...
    # search
    s = admin_session.get(f"{API}/admin/users", params={"q": "creator@qiveo.dev"}, timeout=30).json()
    assert len(s) == 1 and s[0]["email"] == "creator@qiveo.dev"
...
def test_ban_blocks_login_then_unban(admin_session):
    email = f"test_ban_{uuid.uuid4().hex[:8]}@qiveoqa.io"
