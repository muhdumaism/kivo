...
def _cleanup(request):
    ...
    async def run():
        cl = AsyncIOMotorClient(url)
        d = cl[dbname]
        await d.mods.delete_many({"tags": QA_TAG})
        await d.mods.delete_many({"title": {"$regex": "^TEST_QA3"}})
        await d.collections.delete_many({"name": {"$regex": "^TEST_QA3"}})
        await d.organizations.delete_many({"name": {"$regex": "^TEST_QA3"}})
        await d.notifications.delete_many({"text": {"$regex": "TEST_QA3"}})
        await d.users.delete_many({"email": {"$regex": "^test_newuser_"}})
        cl.close()
