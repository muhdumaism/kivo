...
- JWT (localStorage `qiveo_token`, Bearer). bcrypt. Brute-force lockout keyed on `email|client_ip` (X-Forwarded-For aware). Session revocation via token `iat` vs `session_revoked_at`.
